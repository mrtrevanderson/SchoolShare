...
export const addComment = (
  ...
  axios
    .patch(`/api/posts/${postId}`, { action, commenterId, text, timestamp })
    .then(res =>
      dispatch({
        type: ADD_COMMENT,
        payload: res.data,
...
export const deleteComment = (action, commentId, postId) => dispatch =>
  axios.patch(`/api/posts/${postId}`, { action, commentId }).then(res =>
    dispatch({
      type: DELETE_COMMENT,
      payload: res.data
    }));
...
export const editComment = (action, commentId, postId, text) => dispatch =>
  axios.patch(`/api/posts/${postId}`, { action, commentId, text }).then(res =>
    dispatch({
      type: EDIT_COMMENT,
      payload: res.data
    }));
